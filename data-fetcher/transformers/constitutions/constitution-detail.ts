...
export function transform(
  extractConstitutions: ExtractedConstitutionDetail[]
): ConstitutionDetail[] {
  ...
  extractConstitutions.map((c: ExtractedConstitutionDetail) => {
    ...
    if (index === -1) {
      ...
      constitution[index].chapters = constitution[index].chapters.concat(
        data.chapters
      );
      constitution[index].sections = constitution[index].sections.concat(
        data.sections
      );
    }
  });
...
function _getExistingIndex(
  constitutions: ConstitutionDetail[],
  id: string,
  chapter: string
) {
...
function _constructor(
  constitution: ExtractedConstitutionDetail
): ConstitutionDetail {
  const parts = [
    {
  ...
  return {
    id: constitution.id,
    short_name: constitution.short_name,
